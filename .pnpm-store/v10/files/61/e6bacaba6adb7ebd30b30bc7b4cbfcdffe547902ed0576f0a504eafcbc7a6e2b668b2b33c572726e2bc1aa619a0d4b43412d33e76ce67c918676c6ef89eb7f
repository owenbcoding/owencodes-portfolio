...
var RangeCalendarRoot_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const currentModelValue = (0, vue.computed)(() => require_shared_nullish.isNullish(modelValue.value) ? {
			start: void 0,
			end: void 0
		} : modelValue.value);
		...
		const startValue = (0, vue.ref)(currentModelValue.value.start);
		const endValue = (0, vue.ref)(currentModelValue.value.end);
		...
		(0, vue.watch)([startValue, endValue], ([_startValue, _endValue]) => {
			...
			if (_startValue && _endValue) {
				...
				if (value.start && value.end && (0, __internationalized_date.isEqualDay)(value.start, _startValue) && (0, __internationalized_date.isEqualDay)(value.end, _endValue)) return;
		...
		const startingWeekNumberPerMonth = (0, vue.computed)(() => {
			return grid.value.map((month) => {
				const firstDayOfMonth = (0, __internationalized_date.startOfWeek)(month.rows[0][0], locale.value);
				return Array.from({ length: month.rows.length }).fill(null).map((_, idx) => {
					const firstDayOfWeek = firstDayOfMonth.add({ weeks: idx });
					const thursday = firstDayOfWeek.add({ days: 3 });
					const firstDayOfYear = (0, __internationalized_date.startOfYear)(thursday);
