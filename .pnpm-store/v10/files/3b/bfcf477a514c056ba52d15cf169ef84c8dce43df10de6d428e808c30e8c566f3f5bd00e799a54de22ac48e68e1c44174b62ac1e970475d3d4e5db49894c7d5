...
var CalendarRoot_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const startingWeekNumber = (0, vue.computed)(() => {
			const firstDayOfGrid = (0, __internationalized_date.startOfWeek)(grid.value[0].rows[0][0], locale.value);
			return Array.from({ length: grid.value[0].rows.length }).fill(null).map((_, idx) => {
